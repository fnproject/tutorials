...
Let's create a `dockerfn` directory.
...
Let's switch to this directory.
...
With the `dockerfn` directory containing `Dockerfile` and `func.yaml` you've
...
```sh
Building image fndemouser/dockerfn:0.0.1 
...
Successfully tagged fndemouser/dockerfn:0.0.1
...
with a Dockerfile. Of course this is exactly what happened! 
...
```sh
fndemouser/dockerfn           0.0.1               e38bc07ac18e        2 months ago        1.85kB
```
...
![user input](../../images/userinput.png)
>```sh
> fn deploy --app dockerapp --local
>```

You should see output similar to:

```sh
Deploying dockerfn to app: dockerapp at path: /dockerfn
Bumped to version 0.0.2
Building image fndemouser/dockerfn:0.0.2 
Updating route /dockerfn using image fndemouser/dockerfn:0.0.2...
```

Functions are grouped into applications so by specifying `--app dockerapp`
we're implicitly creating the application `dockerapp` and associating our
function with it.
...
`fndemouser/dockerfn:0.0.2` has been bound by the Fn server to the route
`/dockerfn`.  We'll see how to use the route below.

Note that the containing folder name `dockerfn` was used as the name of the
...
is used. To list the functions included in `dockerapp` we can type:

![user input](../../images/userinput.png)
>```sh
> fn list routes dockerapp
>```

```sh
path		      image				                   endpoint
/dockerfn	  fndemouser/dockerfn:0.0.2	  localhost:8080/r/dockerapp/dockerfn
```

The output confirms that `dockerapp` app contains a `dockerfn` function that
is implemented by the Docker container `fndemouser/dockerfn:0.0.2` which may
be invoked via the specified URL. Now that we've confirmed deployment was 
...
However when you called `dockerapp /dockerfn` the fn server looked up the
`dockerapp` application and then looked up the Docker container image
bound to the `/dockerfn` route.
...
`http://localhost:8080/r/dockerapp/dockerfn`.

Use curl to invoke the function:

![user input](../../images/userinput.png)
>```sh
> curl http://localhost:8080/r/dockerapp/dockerfn
>```

The result is once again the same as before:
...
Congratulations! In this tutorial you have created your first function using 
an existing Docker `hello-world` image, run it locally, deployed it to your 
local Fn server and invoked it over HTTP.
